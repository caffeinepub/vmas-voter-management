...
import {
  ...
  X, Star, MessageSquare, AlertCircle, Copy, Printer, MessageCircle,
} from 'lucide-react';
...
function PrintLabelsDialog({ open, onClose, votersToPrint, votersLabel }: PrintLabelsDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={open => !open && onClose()}>
      <DialogContent className="bg-white max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2" style={{ color: '#0b0854' }}>
        ...
        <div className="space-y-4 py-2">
          <p className="text-sm text-muted-foreground">
            Will print labels for <strong>{votersToPrint.length}</strong> voter{votersToPrint.length !== 1 ? 's' : ''}
            {' '}({votersLabel}).
          </p>
          <div>
            <p className="text-sm font-semibold mb-2" style={{ color: '#0b0854' }}>Select Fields to Include</p>
            <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto border border-border rounded-lg p-3">
              {AVAILABLE_FIELDS.map(f => (
                <div key={f.key as string} className="flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor={`print-field-${f.key as string}`} className="text-sm cursor-pointer">
                    {f.label}
                  </Label>
                </div>
              ))}
          ...
          <p className="text-xs text-muted-foreground">
            {config.selectedFields.length} field{config.selectedFields.length !== 1 ? 's' : ''} selected · A4 paper, 2 labels per row
          </p>
        </div>
        <DialogFooter className="flex gap-2 sm:justify-between">
          ...
          >
            ...
            Print {votersToPrint.length} Label{votersToPrint.length !== 1 ? 's' : ''}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function WhatsAppDialog({ open, onClose, selectedVoters }: WhatsAppDialogProps) {
  ...
  const handleSendAll = () => {
    ...
    if (waLinks.length <= 3) {
      ...
      toast.success(`Opened WhatsApp for ${waLinks.length} voter${waLinks.length !== 1 ? 's' : ''}.`);
      onClose();
    }
      ...
      <DialogContent className="bg-white max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="rounded-lg bg-green-50 border border-green-200 p-3 text-sm text-green-800">
            <strong>{votersWithMobile.length}</strong> of <strong>{selectedVoters.length}</strong> selected voter{selectedVoters.length !== 1 ? 's' : ''} have mobile numbers.
          </div>

          {/* Template selector */}
          {templates.length === 0 ? (
            <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">
              No templates found. Please add templates in the <strong>Messaging</strong> page.
            </div>
          ) : (
            <>
              <div>
                <Label className="text-sm font-semibold mb-1.5 block" style={{ color: '#0b0854' }}>
                  Select Template
                </Label>
                  ...
                  <SelectContent className="bg-white">
                    {templates.map(t => (
                      <SelectItem key={t.id} value={t.id}>
                        {t.name}
                        <span className="ml-2 text-xs text-muted-foreground">({t.genderTag})</span>
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {selectedTemplate && waLinks.length > 3 && (
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-2">
                    Click each link to open WhatsApp with the personalized message.
                  </p>
                  <div className="max-h-48 overflow-y-auto space-y-1.5 border border-border rounded-lg p-2">
                    {waLinks.map(({ voter, url }) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground ml-auto">{voter.mobile}</span>
                      </a>
                    ))}
                  </div>
                </div>
        ...
        <DialogFooter className="flex gap-2 sm:justify-between">
          <Button variant="outline" size="sm" onClick={onClose}>Cancel</Button>
          {templates.length > 0 && selectedTemplate && waLinks.length > 0 && waLinks.length <= 3 && (
            <Button
              size="sm"
          ...
          {templates.length > 0 && selectedTemplate && waLinks.length > 3 && (
            <span className="text-xs text-muted-foreground self-center">Click links above to send</span>
          )}
          {templates.length > 0 && votersWithMobile.length === 0 && (
            <span className="text-xs text-destructive self-center">No voters have mobile numbers</span>
          )}
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default function VotersListPage({ onNavigate }: VotersListPageProps) {
  ...
  const votersPrintLabel = selectedIds.size > 0
  ...
  const handlePageJump = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
