...
import { getAllVoters, filterVoters } from '../store/voters';
...
function LabelPreview({ voter, fields }: LabelPreviewProps) {
  ...
  return (
    <div className="border border-border rounded-lg p-3 bg-white text-gray-900" style={{ minHeight: '120px', fontSize: '11px' }}>
      {nameField && (
        <div className="font-bold text-sm border-b border-gray-200 pb-1 mb-1.5">{voter.fullName}</div>
      )}
        ...
        {otherFields.map(fk => {
          ...
          return (
            <div key={fk} className="flex gap-1.5">
              <span className="font-semibold text-gray-500 min-w-[80px]">{def.label}:</span>
              <span>{val}</span>
            </div>
          );
        })}
...
export default function LabelPrintPage() {
  ...
  const [voterScope, setVoterScope] = useState<'all' | 'filtered' | 'sample'>('all');
    ...
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Generate printable voter labels</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold">Select Fields for Labels</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2.5">
                {AVAILABLE_FIELDS.map(f => (
                  <div key={f.key as string} className="flex items-center gap-3">
                    ...
                    />
                    <Label htmlFor={`field-${f.key as string}`} className="text-sm cursor-pointer">
                      {f.label}
                    </Label>
                  </div>
                ))}
              </div>
              <Button variant="outline" size="sm" className="mt-4 gap-2" onClick={handleSaveConfig}>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold">Voter Selection</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                <div>
                  <Label htmlFor="voterScope" className="text-sm font-medium mb-1.5 block">Print Scope</Label>
                  <Select value={voterScope} onValueChange={v => setVoterScope(v as typeof voterScope)}>
                    <SelectTrigger id="voterScope" className="h-9">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">All Voters ({allVoters.length})</SelectItem>
                      <SelectItem value="filtered">Filter by Category</SelectItem>
                ...
                {voterScope === 'filtered' && (
                  <div>
                    <Label htmlFor="printCategory" className="text-sm font-medium mb-1.5 block">Category</Label>
                    <Select value={filterCategory} onValueChange={setFilterCategory}>
                      <SelectTrigger id="printCategory" className="h-9"><SelectValue placeholder="Select category" /></SelectTrigger>
                      <SelectContent>
                ...
                <p className="text-sm text-muted-foreground">
                  {selectedVoters.length} voter{selectedVoters.length !== 1 ? 's' : ''} selected for printing
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold">Label Preview</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              {config.selectedFields.length === 0 ? (
                ...
                <div className="max-w-sm">
                  <LabelPreview voter={sampleVoter} fields={config.selectedFields} />
                </div>
              )}
