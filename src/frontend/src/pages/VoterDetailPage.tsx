...
import { deleteVoter, getVoterById } from "../store/voters";
  ...
  return (
    <div className="flex gap-0.5 items-center">
      {Array.from({ length: 5 }, (_, i) => (
        <Star
          key={`detail-star-${i + 1}`}
          className={`w-4 h-4 ${i < (value || 0) ? "star-filled fill-current" : "star-empty"}`}
        />
      ))}
      {value ? (
...
function Field({
  label,
  value,
}: { label: string; value?: string | boolean | number | null }) {
    ...
    return (
      <div className="p-6 flex items-center justify-center h-64">
        <div className="text-center text-muted-foreground">
          <p>Voter not found.</p>
          <Button variant="link" onClick={() => onNavigate("voters")}>
            Back to list
          </Button>
        </div>
      </div>
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <SectionCard title="Personal Information">
        <dl className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <Field
            ...
            value={
              voter.dateOfBirth
                ? format(new Date(voter.dateOfBirth), "dd MMM yyyy")
                : undefined
            }
          />
      ...
      <SectionCard title="Professional Information">
        <dl className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <Field
            ...
            value={voter.professionalCategory}
          />
          ...
          <div className="flex flex-wrap gap-4">
            {voter.signatureUrl && (
              <div>
                ...
                <img
                  src={voter.signatureUrl}
                  alt="Signature"
                  className="h-16 border rounded object-contain"
                />
              </div>
            )}
            {voter.idProofUrl && (
      ...
      {customFields.length > 0 && (
        <SectionCard title="Custom Fields">
          <dl className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {customFields.map((cf) => {
              const cv = voter.customValues.find(
                (v) => v.fieldId === cf.fieldId,
              );
              return (
                <CustomFieldValue
                  key={cf.fieldId}
      ...
      <div className="text-xs text-muted-foreground flex gap-6 pb-6">
        <span>
          Created: {format(new Date(voter.createdAt), "dd MMM yyyy, HH:mm")}
        </span>
        <span>
          Updated: {format(new Date(voter.updatedAt), "dd MMM yyyy, HH:mm")}
        </span>
      </div>
      ...
      <AlertDialog open={showDelete} onOpenChange={setShowDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{voter.fullName}</strong>?
