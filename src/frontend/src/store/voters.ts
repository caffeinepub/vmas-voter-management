...
import type { VoterFilterState, VoterRecord } from "./types";
...
export function addVoter(
  data: Omit<VoterRecord, "id" | "createdAt" | "updatedAt">,
): VoterRecord {
...
export function filterVoters(filters: VoterFilterState): VoterRecord[] {
  ...
  return voters.filter((v) => {
    ...
    if (
      boothNumber &&
      !v.boothNumber?.toLowerCase().includes(boothNumber.toLowerCase())
    )
      return false;
    if (ward && !v.ward?.toLowerCase().includes(ward.toLowerCase()))
      return false;
    if (education && v.education !== education) return false;
    if (
      profession &&
      !v.profession?.toLowerCase().includes(profession.toLowerCase())
    )
      return false;
    if (categoryLabel && v.categoryLabel !== categoryLabel) return false;
    if (gender && v.gender !== gender) return false;
    if (caste && !v.caste?.toLowerCase().includes(caste.toLowerCase()))
      return false;
    if (
      organizationName &&
      !v.organizationName
    ...
    if (birthdayMonth) {
      ...
      if (month !== Number.parseInt(birthdayMonth)) return false;
    }
...
export function exportToCSV(voters: VoterRecord[]): string {
  ...
  const escapeCSV = (
    val: string | undefined | null | boolean | number,
  ): string => {
